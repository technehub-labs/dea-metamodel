...
Runs three classes of checks:
  ...
  A) Local schema-graph consistency:
     - Every schema in schemas/entities/ has a corresponding
...
def check_cross_repo(graph):
    ...
    for repo in catalog_repos:
        if not github_repo_exists(repo):
            ...
            continue
        # OK - don't print every repo to keep output manageable
    ...
    print(f"C2: Verifying catalog metamodel-pointer.yaml files reference valid entity_ids...")
    broken_pointers = 0
    for repo in catalog_repos:
        content = github_fetch_file(repo, "metamodel-pointer.yaml")
        if not content:
            warnings.append(f"catalog repo '{repo}' has no metamodel-pointer.yaml")
            continue
        ...
        if not claimed:
            warnings.append(f"catalog repo '{repo}' metamodel-pointer.yaml has no entity_id claim")
            continue
        for cid in claimed:
    ...
    if broken_pointers == 0:
        print(f"  ✓ all catalog pointers reference valid entity_ids")
...
def main():
    ...
    try:
        ...
        cross_errors, cross_warnings = check_cross_repo(graph)
        errors.extend(cross_errors)
        warnings.extend(cross_warnings)
        if cross_warnings:
            print(f"Warnings: {len(cross_warnings)}")
            for w in cross_warnings: print(f"  ⚠ {w}")
