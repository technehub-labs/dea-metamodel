...
| [CR-AM-07](./change-requests/CR-AM-07.md) | CR-AM-06 can say *who is comparable* but not *what the comparison shows* — its §10 boundary deliberately withheld percentile, rank, quartile, top-performer, and peer-position. | **Comparative Benchmarking** — `BenchmarkComparison` as a reproducible, governed derivation over an admitted cohort: distribution statistics, per-member percentile, rank + peer position (4/27), deterministic tie rules, minimum-sample enforcement, missing-data-is-N/A. Eligibility consumed, never redefined; ranking is derived, never stored as truth. Phase 1 (comparison vocabulary + `benchmark-comparison.schema.json` + worked 27-member example) implemented; Phases 2–4 per §11. |
